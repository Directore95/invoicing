  ...
  return (
    <Layout>
      <div className="flex justify-between items-center mb-8">
        <div>
          <h1 className="text-2xl font-bold text-gray-900 dark:text-white">{t(locale, 'company.title')}</h1>
      ...
      {companies.length === 0 ? (
        <div className="bg-white dark:bg-slate-800 rounded-xl border border-gray-200 dark:border-slate-700 p-12 text-center">
          <svg className="w-12 h-12 mx-auto text-gray-300 dark:text-slate-600 mb-4" fill="none" viewBox="0 0 24 24" strokeWidth={1} stroke="currentColor">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {companies.map((company) => (
            <div key={company.id} className="bg-white dark:bg-slate-800 rounded-xl border border-gray-200 dark:border-slate-700 p-6 hover:shadow-md transition-shadow">
              ...
              <div className="space-y-1 text-sm text-gray-600 dark:text-slate-400 mb-4">
                <p>{company.street}</p>
                <p>{company.city}{company.state ? `, ${company.state}` : ''} {company.zip}</p>
                {company.email && <p className="text-gray-400 dark:text-slate-500">{company.email}</p>}
                {company.taxId && (
                ...
                {company.registrationNumber && locale === 'sk' && (
                  <p className="text-gray-400 dark:text-slate-500">IČO: {company.registrationNumber}</p>
                )}
              </div>
              ...
              <div className="flex items-center gap-2 pt-4 border-t border-gray-100 dark:border-slate-700">
                <Link
                  ...
                  className="flex-1 text-center text-sm text-blue-600 dark:text-blue-400 hover:bg-blue-50 dark:hover:bg-blue-900/20 py-1.5 rounded-lg transition-colors"
                >
                ...
                {!company.isDefault && (
                  <button
                    ...
                    className="flex-1 text-center text-sm text-gray-500 dark:text-slate-400 hover:bg-gray-50 dark:hover:bg-slate-700 py-1.5 rounded-lg transition-colors"
                  >
                ...
                <button
                  ...
                  className="text-sm text-red-500 dark:text-red-400 hover:bg-red-50 dark:hover:bg-red-900/20 py-1.5 px-3 rounded-lg transition-colors"
                >
